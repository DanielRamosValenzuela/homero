...
this repo at \`scripts/homero/homero.mjs\`. Every \`homero <command>\`
mentioned below means \`node scripts/homero/homero.mjs <command> --target .
...
function validateCliCopy(targetRoot, errors) {
  ...
  if (!fs.existsSync(cliPath)) {
    errors.push("Missing required file: scripts/homero/homero.mjs — re-run `homero init` (it copies the CLI itself into the target repo, it is not a devDependency)");
    return;
  }
