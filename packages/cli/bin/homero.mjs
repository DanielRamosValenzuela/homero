...
- Cross-step layout and summary widgets live under the path recorded in \`homero.config.json\` \`paths.widgetsRoot\`.
...
- Two write-transport patterns are valid; record the choice in \`homero.config.json\` \`transport.pattern\`: **server-actions** (typed server actions/handlers) or **proxy-middleware** (a self-contained edge/proxy layer for route gating, trace-id propagation, and cookies that must not import application modules).
