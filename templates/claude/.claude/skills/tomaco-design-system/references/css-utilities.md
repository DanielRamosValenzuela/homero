...
- **The npm package** (`tomaco-components`, your lockfile) — governs **components
  ...
  for consumers (see `rules/tomaco.md` — "Los estilos NO vienen del paquete npm").
...
is the single most common trap in the library — see `rules/tomaco.md` and
`references/component-gotchas.md` for the exact cascade reason and the
Bootstrap-scale `px-{0-5}` naming collision below.
...
would silently drift from. `rules/tomaco.md` documents the same rule.
...
`../seguros-falabella-ui-ux/references/foundation-summary.md` — this file only
...
`../seguros-falabella-ui-ux/references/foundation-summary.md`. Do not assume a
...
(Bootstrap-scale) fires at **576px**. Same `sm` label, 192px apart. `rules/tomaco.md`
already flags this collision for the directional-spacing case; this table is
the exhaustive version covering every class family, not just spacing.
...
- **Spacing (Bootstrap rem scale, 0-5)**: `m-{0-5}`, `m-auto`, `mx-`/`my-`/`mt-`/
  ...
  - **`px-{0-5}` collides with Tomaco-native `px-{1-100}` (font-size).** For values
    ...
    order documented in `rules/tomaco.md`/`references/component-gotchas.md`. For
    values 0–5 the two rules are adjacent in the cascade; do not assume `px-3`
    behaves as spacing without checking the compiled `tomaco-ui.css` for the
    class you're about to ship, and prefer the unambiguous directional classes
    (`pt{N}`/`pl{N}`/etc. or `pt-{0-5}`/`pl-{0-5}` fully spelled out) over the
    shorthand `px-*`/`py-*` forms when it matters.
...
`references/component-gotchas.md` remain the primary reference — do not tell a
human you queried a tool that isn't there.
